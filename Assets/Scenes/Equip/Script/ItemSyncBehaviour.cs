...
public class ItemSyncBehaviour : MonoBehaviour
{
    ...
    void reload()
    {
        ...
        //APIをたたく
        APIConnectManager.Instance.SyncGetPrice(base_equip.user_item_id, entry.user_item_id, false, (string json) =>
        {
            ...
            //ナビセリフ
            TextNavi.text =  Utility.getText("TEXT_NAVI_EQUIP_SYNCRO_CONFIRM").Replace("{0}", entry.item_name);
    ...
    public void onSync()
    {
        ...
        //合成を行う
        APIConnectManager.Instance.EquipSync(charaId, entry.user_item_id, mount, (string json) =>
        {
            //API結果受け取り
            ...
            if (response.result == "ok")
            {
                ...
                ItemSyncResult.gameObject.SetActive(true);
                ItemSyncResult.Show(base_equip, entry, response);
            ...
            else
            {
                switch (response.err_code)
                {
                    case "noitem":
                    case "not_me":
                        Main.Instance.showDialogue(Utility.getText("API_ERROR_EquipSync_" + response.err_code), null, 4);
                        break;
                    case "equipping":
                    case "maxlevel":
                    case "nomoney":
                    case "in_quest":
                        Main.Instance.showDialogue(Utility.getText("API_ERROR_EquipSync_" + response.err_code));
                        break;
                }
