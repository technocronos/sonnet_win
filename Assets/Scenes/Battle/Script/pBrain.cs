...
public class pBrain : MonoBehaviour
{
    ...
    private static pBrain instance;
    ...
    public void run()
    {
        ...
        Debug.Log("pBrain run run..");

        // ビームを発射してから次のビームを発射できるまでの
        // フレーム数(つまりビームのモーション全体の長さ)を取得。
        BeamBehaviour.Instance.SetSide("P");
        ...
        if (ActRunStep2)
        {
            ...
            else
            {
                ...
                if (reflexCount > 0)
                {
                    ...
                    if (BattleBehaviour.Instance.battle.playerBrainLv >= 50 && RevengePhaseBehaviour.Instance.Launch[target].fireWait < 0)
                    {
    ...
    public void notify()
    {
        ...
        if (reflexCount <= 0)
        {
            ...
            if (!(BattleBehaviour.Instance.battle.playerBrainLv >= 100 && RevengePhaseBehaviour.Instance.Launch[launcher].fireWait < chargeCount + igniteFrames))
            {
    ...
    void setTarget()
    {
        Debug.Log("pBrain setTarget run..");

        // ブレインLv0の場合の、ビーム発射までの反射時間。
        REFLEX_SECS = 1.0 * (100 - BattleBehaviour.Instance.battle.playerBrainLv) / 100;

        targetNo = RevengePhaseBehaviour.Instance.Launch[target].padNo;

        // 何フレーム後に迎撃するかを設定。
        reflexCount = (int)(REFLEX_SECS * (BattleBehaviour.FRAME_RATE)) + 1;
    ...
    void search()
    {
        ...
        for (int i = 0; i < RevengePhaseBehaviour.Instance.LAUNCHER_NUM; i++)
        {
            ...
            // ブレインLv100以上ならエスパースルー判定。
            // 発射までの時間が、次に迎撃可能な時間を下回っているものはスルー。
            if (BattleBehaviour.Instance.battle.playerBrainLv >= 100 && (RevengePhaseBehaviour.Instance.Launch[lnc].fireWait < chargeCount + igniteFrames))
            {
                continue;
            }
